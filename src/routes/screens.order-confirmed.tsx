...
export const Route = createFileRoute("/screens/order-confirmed")({ component: OrderConfirmed });
      ...
      <div className="mx-auto max-w-3xl px-6 py-16 text-center">
        ...
        <div className="surface-card p-6 mt-8 text-left">
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            {["Confirmée", "Préparation", "Livraison"].map((s, i) => (
        ...
        <div className="flex gap-3 mt-6 justify-center flex-wrap">
          <Link to="/screens/tracking" className="btn-orange rounded-xl px-5 py-3 text-sm font-bold flex items-center gap-2">
            <Package className="h-4 w-4" /> Suivre ma commande
          </Link>
          <button className="rounded-xl border border-border bg-card px-5 py-3 text-sm font-semibold flex items-center gap-2">
            <Download className="h-4 w-4" /> Télécharger la facture
          </button>
        </div>
        <Link to="/screens/home" className="mt-4 inline-block text-sm text-muted-foreground hover:text-navy underline">
