...
export const Route = createFileRoute("/screens/checkout-address")({ component: CheckoutAddress });
...
export function Summary({ step }: { step: number }) {
  ...
  const nextTargets = ["/screens/checkout-delivery", "/screens/checkout-payment", "/screens/order-confirmed", "/screens/order-confirmed"];
  return (
    <aside className="surface-card p-6 h-fit sticky top-6">
      ...
      <Link to={nextTargets[step - 1] as any} className="btn-orange w-full rounded-xl py-3 text-sm font-bold mt-4 block text-center">
        {nextLabels[step - 1]}
      </Link>
    </aside>
  );
